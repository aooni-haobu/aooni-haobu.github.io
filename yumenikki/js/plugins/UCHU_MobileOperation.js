...
@param Num1Btn Image
@desc 数字1ボタン画像のファイルパス
@default ./img/system/numbutton1.png

@param Num3Btn Image
@desc 数字3ボタン画像のファイルパス
@default ./img/system/numbutton3.png

@param Num5Btn Image
@desc 数字5ボタン画像のファイルパス
@default ./img/system/numbutton5.png

@param Num9Btn Image
@desc 数字9ボタン画像のファイルパス
@default ./img/system/numbutton9.png
...
@param NumBtn Visible
@desc 数字ボタンを表示する:true しない:false  初期値:true
@default true

@param NumBtn Size
@desc 数字ボタンの大きさ(px）。 初期値:100
@default 100
...
@requiredAssets img/system/numbutton9
@requiredAssets img/system/numbutton5
@requiredAssets img/system/numbutton3
@requiredAssets img/system/numbutton1
...
(function() {
	...
	PRM.url[3] = String(Parameters["Num1Btn Image"]);
	PRM.url[4] = String(Parameters["Num3Btn Image"]);
	PRM.url[5] = String(Parameters["Num5Btn Image"]);
	PRM.url[6] = String(Parameters["Num9Btn Image"]);
	...
	PRM.visible[3] = Boolean(Parameters["NumBtn Visible"] === 'true' || false);
	PRM.size[3] = Number(Parameters["NumBtn Size"]);
...
//	var btn_id=["DirPad","ok","escape"];
	var btn_id=["DirPad","ok","escape","one","three","five","nine"];
	var current_zoom=1;	
	var st_x = 0;
	var st_y = 0;
	...
	function Locate_NumButton() {
		this.initialize.apply(this, arguments);
	}
	Locate_NumButton.prototype.initialize = function(type) {
		var img 	= new Image();
		var url 	= PRM.url[type];
		img.onerror = function() {Graphics.printError('Button Image was Not Found:',url);};
		img.src 	= url;
		...
		//数字キー１
		if (type === 3) {
			this.Div.style['right'] = 10 + PRM.size[3] * 3 + 'px';
			this.Div.style['top'] 	= 10 + 'px';
		//数字キー３
		} else if (type === 4) {
			this.Div.style['right'] = 10 + PRM.size[3] * 2 + 'px';
			this.Div.style['top'] 	= 10 + 'px';
		//数字キー５
		} else if (type === 5) {
			this.Div.style['right'] = 10 + PRM.size[3] + 'px';
			this.Div.style['top'] 	= 10 + 'px';
		//数字キー９
		} else {
		...
		this.Div.style.width 		= PRM.size[3]+'px';
		this.Div.style.height 		= PRM.size[3]+'px';
		this.Div.style.opacity 		= PRM.opacity;
		this.Div.style.zIndex 		= '11';
	...
	//数字キーを非表示
	Locate_NumButton.hideNumButton = function() {
		if (Utils.isMobileDevice() || PRM.pcBtn) {
/*
			if(PRM.visible[3]){document.getElementById("oneBtn").style.zIndex 	= '0';}
			if(PRM.visible[3]){document.getElementById("threeBtn").style.zIndex = '0';}
			if(PRM.visible[3]){document.getElementById("fiveBtn").style.zIndex 	= '0';}
			if(PRM.visible[3]){document.getElementById("nineBtn").style.zIndex 	= '0';}
*/
			if(PRM.visible[3]){document.getElementById("oneBtn").style.visibility 	= 'hidden';}
			if(PRM.visible[3]){document.getElementById("threeBtn").style.visibility = 'hidden';}
			if(PRM.visible[3]){document.getElementById("fiveBtn").style.visibility 	= 'hidden';}
			if(PRM.visible[3]){document.getElementById("nineBtn").style.visibility 	= 'hidden';}
		}
	};
	//数字キーを表示
	Locate_NumButton.showNumButton = function() {
		if (Utils.isMobileDevice() || PRM.pcBtn) {
/*
			if(PRM.visible[3]){document.getElementById("oneBtn").style.zIndex 	= '11';}
			if(PRM.visible[3]){document.getElementById("threeBtn").style.zIndex = '11';}
			if(PRM.visible[3]){document.getElementById("fiveBtn").style.zIndex 	= '11';}
			if(PRM.visible[3]){document.getElementById("nineBtn").style.zIndex 	= '11';}
*/
			if(PRM.visible[3]){document.getElementById("oneBtn").style.visibility 	= 'visible';}
			if(PRM.visible[3]){document.getElementById("threeBtn").style.visibility = 'visible';}
			if(PRM.visible[3]){document.getElementById("fiveBtn").style.visibility 	= 'visible';}
			if(PRM.visible[3]){document.getElementById("nineBtn").style.visibility 	= 'visible';}
		}
	};
	...
	Game_Interpreter.prototype.command351 = function() {
		_Game_Interpreter_command351.call(this);

		if (!$gameParty.inBattle()) {
			//数字キーを非表示
			Locate_NumButton.hideNumButton();
		}
		return true;
	};
	...
	Game_Interpreter.prototype.command352 = function() {
		_Game_Interpreter_command352.call(this);
	...
	Game_Interpreter.prototype.showNumButton = function() {
		Locate_NumButton.showNumButton();
	};
	Game_Interpreter.prototype.hideNumButton = function() {
		Locate_NumButton.hideNumButton();
	};
	...
	Scene_Menu.prototype.createCommandWindow = function() {
		...
		//キャンセルが押された時の処理を上書きする
		this._commandWindow.setHandler('cancel', function(){
			this.popScene();
	...
	Scene_File.prototype.createListWindow = function() {
		...
		//キャンセルを押された時の処理を上書きする
		this._listWindow.setHandler('cancel', function(){
			this.popScene();
	...
	Scene_Base.prototype.start = function() {
		...
		if (Utils.isMobileDevice() || PRM.pcBtn) {
			if(!Btn_ready){
				...
				if(PRM.visible[3]){this.oneButton = new Locate_NumButton(3);}
				if(PRM.visible[3]){this.threeButton = new Locate_NumButton(4);}
				if(PRM.visible[3]){this.fiveButton = new Locate_NumButton(5);}
				if(PRM.visible[3]){this.nineButton = new Locate_NumButton(6);}
...
//	if(PRM.visible[0] || PRM.visible[1] || PRM.visible[2]){
	if(PRM.visible[0] || PRM.visible[1] || PRM.visible[2] || PRM.visible[3]){
		...
		Graphics._updateRealScale = function() {
			...
			if (this._stretchEnabled) {
				...
				if(window.innerWidth<window.innerHeight){current_zoom=hvzoom[1];}else{current_zoom=hvzoom[0];}
					...
					if(PRM.visible[3]){document.getElementById("oneBtn").style.zoom=current_zoom;}
					if(PRM.visible[3]){document.getElementById("threeBtn").style.zoom=current_zoom;}
					if(PRM.visible[3]){document.getElementById("fiveBtn").style.zoom=current_zoom;}
					if(PRM.visible[3]){document.getElementById("nineBtn").style.zoom=current_zoom;}
				}
			}
		};
	}
