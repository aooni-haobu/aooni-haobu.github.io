...
/*:
 * @plugindesc set various effects to events and player
 ...
 */
/*:ja
 * @plugindesc イベントやプレイヤーに様々な効果をセットします
 * @author 神無月サスケ
 *
...
(function() {
  ...
  Game_CharacterBase.prototype.tint = function(tone, duration, needsWait) {
    ...
    if (needsWait) {
      ...
      this._waitCount = duration; // *** added
    }
  };
  ...
  Sprite_Character.prototype.setFrame = function(sx, sy, pw, ph) {
    if (this._character._offsets2) {
      ...
      var sry = frameOffsets.y + frameOffsets.height;
      pw -= Math.min(Math.max(pw - srx, 0) + frameOffsets.x + Math.abs(offsets2.x), pw);
      ph -= Math.min(Math.max(ph - sry - Math.max(-offsets2.y, 0), 0) + frameOffsets.y + Math.abs(offsets2.y), ph);
      console.log(pw,ph);
    }
  ...
  Game_CharacterBase.prototype.setImageGrid2 = function(name, index, dir, pattern, duration, needsWait) {
    if (duration > 0) {
  ...
  Game_CharacterBase.prototype.setImage2 = function (name, index, duration,  needsWait) {
    if (duration > 0) {
      this._newIgName = name;
      this._newIgIndex = index;
  ...
  Game_CharacterBase.prototype.setGrid2 = function(dir, pattern, duration, needsWait) {
    if (duration > 0) {
